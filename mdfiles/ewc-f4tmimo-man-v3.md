...
This manual provides information for the use of ESPEC Web Controller (EWC), Version 3.3. EWC is capable of communicating and controlling the following process controllers and programmable logic controllers (PLCs): ESPEC P300, SCP220, ES102, Watlow F4 and F4T, Allen Bradley CompactLogix, ControlLogix and Micro8xx series. Communication can be established via an Ethernet TCP/IP protocol through an Ethernet port or via an RS-232/RS-485 through a serial or USB-to-Serial interface.
...
This manual focuses on ESPEC custom chambers equipped with Watlow F4T controller and touchscreen (called MIMO). 
...
The PDF format is available as an online viewable from within EWC, accessible under the Manual tab in the **About** menu.   
...
The following figure depicts a typical home page of EWC displaying the system bar designated and labeled as item **1**. Components and functionality of the system bar are described as follows: 
...
1. **System bar**: EWC on its HMI has the system bar (at the top) to provide the touchscreen operation as shown above. It includes the URL that displays its current menu in the middle, three icons on the right (labeled as **Screen shot**, **Network Info** and **USB Device**) described as follows:    

    ![System bar and operational options](https://bitbucket.org/repo/kMrjrgg/images/184221221-main-touchscreen-menu-bar-001c.PNG)

    * **URL/Menu**: To access the URL menu bar, touch the middle area of the system bar indicated by the arrow. This URL displays the current menu of EWC. It provides an extended operation of EWC menu navigation, such as backward or forward menu, refresh the UI page, or access a new menu. It is probably easier to access a different menu via the menu bar (on the left via the touch operation), but the URL field accepts input of a new menu by entering its name and pressing the right arrow. 
...
**Note**: If you click **SKIP** on any of the above **Setup Wizard** pages, they will reappear every time you restart and log into EWC.  
...
To log into a specific account on EWC via HMI (touchscreen), use the touchscreen keyboard as shown below. 

![Log into EWC via its HMI](https://bitbucket.org/repo/kMrjrgg/images/2756890753-onscreen-login-001a.PNG)
...
**Note**: The Network Monitor page is not operational on a monitor (touchscreen or standard display) that is connected directly to EWC via its HDMI or display port. It can only be accessed and controlled from a Web-based device on the local network. For this reason, the touchscreen of a T-series or custom F4T chamber cannot be used to set up the Network Monitor page. 
...
In this section, we provide a step-by-step example how to set up and configure a network monitor page to display three EWC devices in the default Monitor tab. This example can be completed only if you have at least two EWC devices, one as a host and the other as a client on your network. Note: All EWC device hostnames and IP addresses used in this example are for illustration purposes only. You must use the hostnames and IP address of your EWC devices found on your network. 
...
The **Overview** page displays the current status of the chamber and its operating mode. A user is brought to this page after successfully logging into EWC. The following figure depicts **Overview** with the chamber in Standby as indicated in the status tab and its extension bar. This extension bar is only available in the **Overview** menu. 

The date/time (labeled as item 2) posted in the extension bar is the date/time on the chamber/controller. The date/time (labeled as item 1) posted in the extension bar is the date/time on the chamber/controller. While date/time stamp of data logs and Macros automated actions are based on the date/time on EWC, the controller date/time should also synchronize with that on EWC and the local device (that launched the Web browser). The date/time of EWC can be viewed under the **Settings** menu. It can also synchronize with that on the local device (to be discussed in the **Settings** menu). 

![Overview page with chamber in Standby mode](https://bitbucket.org/repo/kMrjrgg/images/1482852419-ewcv3-touch-f4t-overview-001ab.PNG)

The following figure shows that the date/time of EWC (upper left, item 1) and the chamber/controller (in the extension bar, item 2) is almost synchronized. 

![Chamber/controller and EWC date/time is synchronized](https://bitbucket.org/repo/kMrjrgg/images/1403599908-ewcv3-touch-f4t-overview-002a.PNG)

The following figure depicts **Overview** showing the chamber in Constant mode. 

![Overview page with chamber in Constant mode](https://bitbucket.org/repo/kMrjrgg/images/2644950270-touch-overview-const-001.png)
...
![Overview page with chamber in Program mode](https://bitbucket.org/repo/kMrjrgg/images/3150580556-touch-overview-prog-001.png)
...
A repeating beep on the local computer (that runs a Web browser to control EWC) is also tripped to get the operator's attention. The **SILENCE** button can be used to turn off the beep. This alert window can be closed by clicking the **CLOSE** button or the X button. However, the alarm state still remains to be resolved as indicated by the **Status** tab in the following figure. To redisplay or expand the alarm list, click the red dot in the lower-right corner.   

![Alarm state in overview page](https://bitbucket.org/repo/kMrjrgg/images/1870705789-overview-alarm-01c.PNG)

On a touchscreen monitor, these triggered alarms can be relisted by touching the red dot on the lower-right corner. 

![Alarm state displayed on MIMO touchscreen](https://bitbucket.org/repo/kMrjrgg/images/1922557676-mimo-touchscreen-001.png)

In an alarm state, operation is halted until all alarms triggered by the chamber are resolved. The chamber operation mode may be switched to a constant mode, as depicted in the following figure; however, all active alarms are still unresolved, as indicated in the extended status tab. Once all alarms are cleared, EWC automatically clears all alert messages and resumes in the **Constant** mode. 

![Constant mode may be switched during an alarm state](https://bitbucket.org/repo/kMrjrgg/images/4183465784-mimo-alarm-cnst-001.png)

If alarms occurred during a program execution state, operation is halted in an alarm state. Once all alarms are cleared, EWC automatically clears all alert messages and resumes normal operation by switching the chamber to a **Standby** mode, as depicted in the following figure.

![Standby mode after alarms are cleared](https://bitbucket.org/repo/kMrjrgg/images/3502458518-mimo-stndby-mode-alarm-001.PNG)
...
With **PROG15TEST** program open in the program editor, as shown in the previous figure, complete the following steps to modify and add additional steps with their specific step type and parameters as follows:
   ...
   5. **Step 4**: Modifying the current step.  
       ...
       * **Duration**: Grayed out (to be determined by EWC on temperature set value and rate).
       * **Temperature**: 
...
Contents in the trend graph discussed in the **Trend** menu are dictated by the settings on this page. Data points collected from the chamber will bear a date/time stamp of EWC, shown in the upper-left in the system bar, regardless of the date/time on the controller (see the extension bar of the status tab in **Overview**). For practical purposes, date/time of EWC should reflect the current date and local time zone.   

Two panes for data logging are available to support the configuration (as depicted in the following figure). They are described as follows: 
